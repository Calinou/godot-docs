...
This section of the tutorial covers (2D) GPU-accelerated particle systems. Most of the things
...
.. figure:: img/particle_nodes.webp
   :alt: A list of nodes related to 2D particles
   :align: right

   All 2D particle nodes available in Godot

There are two types of 2D particle systems in Godot: :ref:`class_GPUParticles2D`, which are processed on the GPU,
and :ref:`class_CPUParticles2D`, which are processed on the CPU.
...
There are several particle collision nodes. :ref:`class_GPUParticlesCollisionBox2D` and
:ref:`class_GPUParticlesCollisionSphere2D` are the simple ones. You can use them to create basic
shapes like boxes, a floor, or a wall that particles collide with. The other two nodes provide
more complex collision behavior. The :ref:`class_GPUParticlesCollisionSDF2D` is useful when you want
...
:ref:`class_GPUParticlesCollisionHeightField2D` node. It creates a heightmap of your world and the
objects in it and uses that for large-scale particle collisions.

.. note::

    Unlike 2D particles, 2D particles currently do not support attractor nodes.
